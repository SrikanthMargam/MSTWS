...
function InstallXpertAgent {
    ...
    Write-Output "Setting up DataCollector.config.xml..."
    ...
    Write-Output "Setting up AgentIdentityConfiguration.xml..."
    ...
    Write-Output "Setting up XpertAgent task script..."
    ...
    Write-Output "Setting up XpertAgentStarter task script..."
    ...
    Write-Output "Setting up Task Scheduler tasks..."
    ...
    If ($processActive -eq $null) {
        Write-Output -backgroundcolor DarkRed $env:COMPUTERNAME, "XpertAgent not started"
    }
    Else {
        Write-Output -backgroundcolor DarkGreen $env:COMPUTERNAME, "XpertAgent started"
    }
...
function DiskConfiguration() {
    try {
        Write-Output "-----Begin-DiskConfiguration------"

        Write-Output "Stopping Shell Detection Service"
        $role = ($Script:Role)
        Stop-Service -Name ShellHWDetection

        Write-Output "Extending C:\ to maximum Size"
        ...
        Write-Output "Initializing All RAW Disks"
        ...
        foreach ($disk in $disks) {
            ...
            Switch ($disk.Number) {
                ...
                6 {
                    ...
                    Write-Output "T:\ Created"
                }
            }
            sleep 5
        }
        Write-Output "Starting Shell Detection Service"
        Start-Service -Name ShellHWDetection
        Write-Output "-----End-DiskConfiguration------"
    }
    Catch [Exception] {
...
function enableIIS() {
    try {
        ...
        Write-Output "Completed Enabling IIS Features"
        Write-Output "-----End-enableIIS------"
        Write-Output ""
    }
    Catch [Exception] {
        Write-Output "Exception Occured..."
...
function RemoveDefaultWebsite() {
    try {
        Write-Output "-----Begin-RemoveDefaultWebsite------"
        Write-Output "Removing Default Website"
        Import-Module WebAdministration
        Remove-WebSite -Name 'Default Web Site'   
        Write-Output "-----End-RemoveDefaultWebsite------"
    }
    Catch [Exception] {
        Write-Output "Exception Occured..."
        Write-Output "Exception $_.Exception.GetType().FullName : $_.Exception.Message"
...
function ConfigureEventLog() {
    try {
        Write-Output "-----Begin-ConfigureEventLog------"
        Write-Output "Creating EveningLog"
        new-EventLog -LogName Application -source 'AzureArmTemplates' -ErrorAction SilentlyContinue
        Write-Output "-----End-ConfigureEventLog------"
    } 
    Catch [Exception] {
        Write-Output "Exception Occured..."
        Write-Output "Exception $_.Exception.GetType().FullName : $_.Exception.Message"
...
function AddFirstSQLAdmin() {
    try {
        ...
        Write-Output "-----Begin-AddFirstSQLAdmin------"
        Write-Output "Stopping SQL Server"
        Stop-Service -Name "MSSQLSERVER" -Force
        $sqlservice = Get-Service "mssqlserver" 
        Write-Output "Starting SQL Server in Single Admin Mode"
        $sqlservice.Start("-m")
        Write-Output "Invoking $SQLFile"
        ...
        Write-Output "Finished Executing $SQLFile"
        Write-Output "Stopping SQL Service"
        Stop-Service -Name "MSSQLSERVER" -Force
        Write-Output "Starting SQL Service with normal mode"
        Start-Service -Name "MSSQLSERVER"
        Write-Output "-----End-AddFirstSQLAdmin------"
    }
    Catch [Exception] {
        Write-Output "Exception Occured..."
        Write-Output "Exception $_.Exception.GetType().FullName : $_.Exception.Message"
...
function DriveCheck($DriveLetter) {
    try {
        ...
        if ($diskArray -eq $nothing) {
            ...
            Write-Output "$DriveLetter - NOT FOUND"
            throw "Drives not available as expected"

        }
        else {Write-Output "$DriveLetter - Drives Ready"}
        Write-Output "-----End-DriveCheck------"
    } 
...
function ConfigureStartupPath() {
    try { 
        Write-Output "-----Begin-ConfigureStartupPath-------"
        $Root = "C:\SQLStartup"
        ...
        if ($(test-path -path $root) -eq $true) {
            ...
            #Setting the Change
            Set-Acl $Root $acl
            Write-Output "-----End-ConfigureStartupPath-------"
...
function ConfigureDataPath() {
    try { 
        Write-Output "-----Begin-ConfigureDataPath-------"
        ...
        if ($(test-path -path $root) -eq $true) {
            ...
            #Setting the Change
            Set-Acl $Root $acl
            Write-Output "-----End-ConfigureDataPath-------"
        }                         
...
function ConfigureLogPath() {
    try { 
        Write-Output "-----Begin-ConfigureLogPath-------"
        $Root = ($Script:logPath)
        CreateFolder($Root)
        if ($(test-path -path $root) -eq $true) {
            ...
            #Setting the Change
            Set-Acl $Root $acl
            Write-Output "-----End-ConfigureLogPath-------"
        }                         
...
function ConfigureTempdbPath() {
    try { 
        Write-Output "-----Begin-ConfigureTempdbPath-------"
        ...
        if ($(test-path -path $root) -eq $true) {
            ...
            #Setting the Change
            Set-Acl $Root $acl
            Write-Output "-----End-ConfigureTempdbPath-------"
        }                         
...
function ConfigureSQLServerService() {
    ...
    Write-Output "-----Begin-ConfigureSQLServerService------"
    ...
    try {
        ...
        Write-Output "Disabling All SQL Related Services expect SQLSERVER, AGENT & WRITER"   
        ...
        Write-Output "Setting SQL Service Accont to Local System"     
        #set sql Service
        ...
        Write-Output "-----End-ConfigureSQLServerService------"
...
function ConfigureSQLAgentService() {
    Write-Output "-----Begin-ConfigureSQLAgentService------"
    Write-Output "Setting SQL Agent Service Accont to Local System"
    ...
    try {
        ...
        $svc.Stop()
        $svc.Start()
        Write-Output "-----End-ConfigureSQLAgentService------"
...
function Add-FirewallRules {
    Write-Output "-----Begin Add-FirewallRules------"
    Write-Output "Adding Inbound Firewall Rules..."
    ...
    New-NetFirewallRule -Name "CAC-I" -DisplayName "TWS_Allow_Cache_Port" -Direction Inbound -LocalPort 22233-22236 -Protocol TCP -Action Allow -ErrorAction SilentlyContinue
    Write-Output "Completed Inbound Firewall Rules"
    Write-Output "Adding Outbound Firewall Rules"
    ...
    New-NetFirewallRule -Name "CAC-O" -DisplayName "TWS_Allow_Cache_Port" -Direction Inbound -LocalPort 22233-22236 -Protocol TCP -Action Allow -ErrorAction SilentlyContinue
    Write-Output "Completed Outbound Firewall Rules"


    Write-Output "-----End-FirewallRules------"
}
...
Function InstallGenevaAgent {
    ...
    Write-Output "-----Begin-InstallGenevaAgent------"
    Write-Output "Copying Geniva Agent to D:\"
...
Function EnableMSDTC {
    ...
    Write-Output "-----End-EnableMSDTC------"
}

Function Set-TimezonetoPST {
    Write-Output "-----Begin-Set-Timezone------"
    Write-Output "Updating Server Timezone to PST"
    Set-TimeZone -Name "Pacific Standard Time"
    Write-Output "-----End-Set-Timezone------"
}
...
function WebStoreSetup() {
    ...
    Write-Output "-----Begin-WebStore Installation------"
	
    $client = Start-Process msiexec.exe -ArgumentList "/i C:\Packages\Plugins\SupportFiles\WebStore8\WstSetup_64.msi DATACENTER=""DC1"" CONFIGSERVERS=""BAYTWSSQLWAW101"" ADDLOCAL=""ManagedClient"" /lv C:\ManagedClient.log /quiet" -Wait -PassThru -NoNewWindow; 
    Write-Output ("------WebStore Installation Status:" + $client.ExitCode)
}

function EnableCredSSP {
    Enable-WSManCredSSP -Role Server -Force
    Enable-WSManCredSSP -Role Client -DelegateComputer "*.phx.gbl" -Force
    Set-Item -Path WSMan:\localhost\Service\auth\credSSP -Value $True -Force
    Set-Item -path WSMan:\localhost\Client\TrustedHosts -Value "*.phx.gbl" -Force
